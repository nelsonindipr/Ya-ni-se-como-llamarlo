...
function App() {
  ...
  return <main className="app-shell">
    ...
    <footer className="footer-controls"><button onClick={() => { saveSeasonState(state); setStatusMessage('Season saved.'); }}>Save</button><button onClick={() => { const loaded = loadSeasonState(); if (loaded) { setState(loaded); setStatusMessage('Season loaded.'); } }}>Load</button><button onClick={() => { clearSeasonState(); const next = createNewGameState(initialSeed); setState(next); setStatusMessage('Season reset.'); }}>New Game</button><button onClick={() => persist(simulateNextPlayoffSeriesForState(state), 'Simulated next playoff series.')} disabled={!state.playoffBracket}>Sim Next Playoff Series</button><small>Rules: {leagueRules.game.numPeriods} x {leagueRules.game.quarterLength} min</small></footer>
  </main>;
}
