...
describe('archetype-based OVR rollout', () => {
  it('elite off-screen shooter selects offscreen_shooter and improves over flat base', () => {
    ...
    expect(diag.bestArchetypeName).toBe('offscreen_shooter');
    expect(diag.finalRoundedOverall).toBeGreaterThan(Math.round(diag.baseAdjustedScore));
  });
  ...
  it('shot creator guard selects shotCreatorSG or scoringPG', () => {
    ...
    expect(['shot_creator_sg', 'scoring_pg']).toContain(sg.bestArchetypeName === 'shot_creator_sg' ? 'shot_creator_sg' : pg.bestArchetypeName);
  });
  ...
  it('defensive guard selects defensivePG or threeAndDSG', () => {
    ...
    const sg = calculatePositionOverallDiagnosticsFromRatings(ratings, 'SG');
    const pg = calculatePositionOverallDiagnosticsFromRatings(ratings, 'PG');
    expect(['three_and_d_sg', 'defensive_pg']).toContain(sg.bestArchetypeName === 'three_and_d_sg' ? 'three_and_d_sg' : pg.bestArchetypeName);
  });
  ...
  it('rebounding guard selects rebounding profile', () => {
    ...
    const sg = calculatePositionOverallDiagnosticsFromRatings(ratings, 'SG');
    const pg = calculatePositionOverallDiagnosticsFromRatings(ratings, 'PG');
    expect(['rebounding_sg', 'rebounding_pg']).toContain(sg.bestArchetypeName === 'rebounding_sg' ? 'rebounding_sg' : pg.bestArchetypeName);
  });
  ...
  it('passing center / stretch big / rim protector select expected center archetypes', () => {
    ...
    expect(passingC.bestArchetypeName).toBe('passing_hub_c');
    ...
    expect(stretchC.bestArchetypeName).toBe('stretch_five');
    ...
    expect(rimC.bestArchetypeName).toBe('rim_protector');
  });

  it('balanced wing remains stable and not over-inflated (dynamic cap)', () => {
    ...
    expect(diag.finalRoundedOverall - Math.round(diag.baseAdjustedScore)).toBeLessThanOrEqual(9);
  });
  ...
  it('applies downside floor (-2) and dynamic uplift cap (+5/+7/+9)', () => {
    ...
    const diag = calculatePositionOverallDiagnosticsFromRatings(ratings, 'SG');
    expect(diag.cappedScore).toBeLessThanOrEqual(base + 9);
    expect(diag.cappedScore).toBeGreaterThanOrEqual(base - 2);
  });
});
